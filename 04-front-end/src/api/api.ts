...
export type ApiRole = 'viewer' | 'administrator';
...
function getToken(role: 'viewer' | 'administrator'): string {
    const token = localStorage.getItem('api_token' + role);
    return 'Bearer ' + token;
}

export function saveToken(role: 'viewer' | 'administrator', token: string) {
    localStorage.setItem('api_token' + role, token);
}

function getRefreshToken(role: 'viewer' | 'administrator'): string {
    const token = localStorage.getItem('api_refresh_token' + role);
    return token + '';
}

export function saveRefreshToken(role: 'viewer' | 'administrator', token: string) {
    localStorage.setItem('api_refresh_token' + role, token);
}

export function saveIdentity(role: 'viewer' | 'administrator', itentity: string) {
    localStorage.setItem('api_identity' + role, itentity);
}

export function getIdentity(role: 'viewer' | 'administrator'): string {
    const token = localStorage.getItem('api_identity' + role);
    return 'Bearer ' + token;
}

export function removeTokenData(role: 'viewer' | 'administrator') {
...
async function refreshToken(role: 'viewer' | 'administrator'): Promise<string | null> {
